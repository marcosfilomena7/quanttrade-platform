...
def _truncate_all_tables(conn: sa.Connection) -> None:
    """`capture_universe_snapshot` (the code under test) commits its own
    writes internally — legitimate production behavior, but it means a
    plain `connection.rollback()` cannot undo them. Truncating every
    table (except `alembic_version`) after each test is what actually
    gives each test a clean slate, regardless of what the code under
    test committed."""
    tables = conn.execute(
        text(
...
def _insert_venue(conn: sa.Connection, *, name: str | None = None) -> uuid.UUID:
    """`name` defaults to a fresh, per-call unique value: `venue.name` has
    a UNIQUE constraint, and the job functions under test commit their
    own writes internally (legitimate production behavior — see
    `capture_universe_snapshot`), so a fixed literal like `"binance"`
    would collide across tests within the same container once one
    test's insert is actually durable rather than rolled back."""
    venue_id = uuid.uuid4()
    conn.execute(
        text(
