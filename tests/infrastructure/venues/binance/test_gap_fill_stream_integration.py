...
def _truncate_all_tables(conn: sa.Connection) -> None:
    """Some job functions this stream's gap-fill path calls into (e.g.
    T-P1-04's `backfill_candles`) commit their own writes internally
...
def _insert_venue(conn: sa.Connection, *, name: str | None = None) -> uuid.UUID:
    ...
    has a UNIQUE constraint, and code this stream calls into commits
    its own writes internally, so a fixed literal like `"binance"`
